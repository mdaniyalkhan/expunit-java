...
public class MethodUtility {
    ...
    public static Object[] getMethodParametersValues(Method methodInfo) {
        ...
        Parameter[] parameterInfos = methodInfo.getParameters();
        Object[] parameters = new Object[parameterInfos.length];
        for (int index = 0; index < parameterInfos.length; index++) {
            Class parameterType = parameterInfos[index].getType();

            if (parameterType == Class.class) {
                parameters[index] = methodInfo.getDeclaringClass();
                continue;
            }
            Object defaultValue = TypeUtility.getValue(parameterInfos[index].getName(), parameterType, index +
                    totalNumberOfFields + 1);
            if (defaultValue != null) {
                parameters[index] = defaultValue;
            } else if (Modifier.isInterface(parameterType.getModifiers()) || Modifier.isAbstract(parameterType
                    .getModifiers())) {
                if (TypeUtility.isACollection(parameterInfos[index].getType()) &&
                        parameterInfos[index].getParameterizedType() instanceof ParameterizedType) {
                    parameters[index] = TypeUtility.getValue((ParameterizedType) parameterInfos[index]
                            .getParameterizedType(), index +
                            totalNumberOfFields + 1);
                }
            } else {
    ...
    public static Method getMethod(MethodInfo methodInfo) {
        ...
        methodInfo.setParameterInfos(result.getParameters());
